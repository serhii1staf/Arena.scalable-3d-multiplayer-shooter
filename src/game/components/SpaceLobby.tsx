...
export const LOBBY_ZONES = {
  ...
  skins: { pos: [15, 0, -4] as [number, number, number], radius: 3, id: "skins" },
  board: { pos: [-15, 0, -4] as [number, number, number], radius: 3.5, id: "board" },
  settings: { pos: [0, 0, 16] as [number, number, number], radius: 2.5, id: "settings" },
};
export const JUMP_PADS = [{ pos: [12, 0, 12] as [number, number, number], radius: 1.6, power: 19 }];
...
function Booth({ position, label, sub, color, children }: { position: [number, number, number]; label: string; sub: string; color: string; children?: React.ReactNode }) {
  return (
    <group position={position}>
  ...
  return (
    <group>
      ...
      <RigidBody type="fixed" colliders={false}>
        <mesh geometry={GEO.cyl} material={toon("#a7f3d0")} scale={[22, 2, 22]} position={[0, -1, 0]} receiveShadow />
        ...
        <mesh geometry={GEO.box} material={toon("#fff7ed")} scale={[3, 0.12, 30]} position={[0, 0.06, -2]} receiveShadow />
        <mesh geometry={GEO.box} material={toon("#fff7ed")} scale={[30, 0.12, 3]} position={[0, 0.06, -4]} receiveShadow />
        {tiles.map(([x, z], i) => (
        ...
        {[
          ...
          <group key={i} position={[x, 0, z]}>
            ...
            <mesh geometry={GEO.sphereLow} material={toon(i % 2 ? "#86efac" : "#67e8f9")} scale={1.6} position={[0, 3, 0]} castShadow />
            <mesh geometry={GEO.sphereLow} material={toon(i % 2 ? "#bbf7d0" : "#a5f3fc")} scale={1} position={[0.7, 3.8, 0.3]} />
        ...
        {/* Настройки */}
        <Booth position={LOBBY_ZONES.settings.pos} label="НАСТРОЙКИ" sub="Нажми E" color="#bae6fd">
          <mesh geometry={GEO.torus} material={toon("#0ea5e9", "#38bdf8", 0.4)} scale={0.9} position={[0, 1.8, 0]} rotation={[0.4, 0, 0]} />
        </Booth>
