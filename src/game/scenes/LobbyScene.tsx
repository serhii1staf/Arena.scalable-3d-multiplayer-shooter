...
import { GradientSky, AdaptiveQuality, Lights } from "../components/Environment";
...
export default function LobbyScene() {
  ...
  // presence heartbeat: 1 запрос / 2 с, позиция + список остальных
  useEffect(() => {
    if (!profile || !started) return;
    let stop = false;
    const tick = async () => {
      try {
        const r = await fetch("/api/lobby/presence", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ playerId: profile.id, name: profile.name, skinId: profile.skinId, room: "main", ...pos.current }),
        });
        const d = await r.json();
        if (!stop && d.others) setOthers(d.others);
      } catch {
        /* offline — просто продолжаем */
      }
    };
    tick();
    ...
    return () => {
      stop = true;
      clearInterval(id);
      fetch("/api/lobby/presence", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ playerId: profile.id }), keepalive: true }).catch(() => {});
    };
  }, [profile, started]);
    ...
    <div ref={wrap} className="fixed inset-0 overflow-hidden bg-sky-100" style={{ touchAction: "none" }}>
      ...
      <div className="pointer-events-none fixed left-0 right-0 top-0 z-20 flex items-start justify-between p-4">
        <div className="pointer-events-auto flex items-center gap-3 rounded-2xl bg-white/85 px-4 py-2 shadow-lg backdrop-blur">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-violet-400 to-fuchsia-400 text-lg font-black text-white">{profile.name[0].toUpperCase()}</div>
          <div>
      ...
      {started && !panel && !input.touch && (
        <div className="pointer-events-none fixed bottom-4 left-1/2 z-20 -translate-x-1/2 rounded-xl bg-white/70 px-3 py-1.5 text-xs font-semibold text-slate-600 backdrop-blur">
          WASD — движение · Мышь — камера · Space — прыжок · E — взаимодействие · Esc — курсор
        </div>
      )}
