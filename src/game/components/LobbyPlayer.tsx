...
export function LobbyPlayer({ skinId, name, onMove, zones, onZone, onInteract, jumpPads = [] }: Props) {
  ...
  useFrame((_, rawDt) => {
    ...
    const dist = 5.5;
    _camPos.set(t.x + Math.sin(s.yaw) * Math.cos(s.pitch) * dist, t.y + 1 + Math.sin(s.pitch) * dist, t.z + Math.cos(s.yaw) * Math.cos(s.pitch) * dist);
    camera.position.lerp(_camPos, 1 - Math.exp(-dt * 12));
    _look.set(t.x, t.y + 1, t.z);
    camera.lookAt(_look);
