...
/**
 * Процедурный мультяшный персонаж: ~14 мешей, общие геометрии, кэшированные материалы.
 * Анимация: ходьба (ноги/руки), дыхание, отдача при выстреле — всё в одном useFrame.
 */
export const Character = forwardRef<CharacterHandle, Props>(function Character({ skinId, castShadow = true, weapon = true }, ref) {
  ...
  useFrame((_, dt) => {
    ...
    // правая рука держит оружие — поднимаем при стрельбе / на бегу
    s.recoil = Math.max(0, s.recoil - dt * 6);
    rArm.current.rotation.x = -1.3 - s.recoil * 0.5 + swing * 0.15 * (1 - s.recoil);
    ...
    <group ref={group}>
      <group ref={body}>
        ...
        <group ref={head} position={[0, 1.32, 0]}>
          ...
          {/* блики глаз */}
          <mesh geometry={GEO.sphereLow} material={basic("#ffffff")} scale={0.05} position={[-0.09, 0.08, 0.36]} />
          <mesh geometry={GEO.sphereLow} material={basic("#ffffff")} scale={0.05} position={[0.09, 0.08, 0.36]} />
